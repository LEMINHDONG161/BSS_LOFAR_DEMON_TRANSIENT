...
cd(Data_path)
disp('Load...')
load('Target_NMF_1'); Target_1 = Target_sig;
...
disp('FD-JADE...')
FD_sep1_F = []; FD_sep2_F = [];FD_sep3_F = []; FD_sep4_F = []; FD_sep_F = cell(1,2,3,4);
for i = 1:size(Ds_STFT{1,1},2)
    ...
    FD_sep4_F = [FD_sep4_F FD_jade(4,:)'];
end
clear i

FD_sep_F{1,1} = FD_sep1_F; FD_sep_F{1,2} = FD_sep2_F;FD_sep_F{1,3} = FD_sep3_F; FD_sep_F{1,4} = FD_sep4_F; % STFT (Seperated)
...
[FD_sep4, T_ifft4] = istft(FD_sep4_F, win, overlap, nfft, dfs);
FD_sep = [FD_sep1; FD_sep2; FD_sep3; FD_sep4]; % Seperated signals
...
[Wa2, Ha2]   = nmf_s(N_FD_sep1_F, 2, [h_W2 randn(size(B))], 100, 1);
    Wa2(:,1) = log((1./Wa2(:,1)));
...
set(gcf,'numbertitle','off','name', 'FD-JADE_transient');
    ...
    set(gca,'fontsize',12)
    set(gcf,'color','w')
    %% Display 20s & 40s FDJADE_Transient information
figure,
plot(F_LOFAR,db(FD_sep_F{1,2}(:,10)),'r-');
...
grid on
figure,
plot(F_LOFAR,db(FD_sep_F{1,2}(:,80)), 'g-');
%plot(F_LOFAR_T,db(XmagHat./max(max(XmagHat)))); % display for target input nnmf
xlabel('Frequency hz ','fontsize',12);
ylabel('Amplitude ','fontsize',12);
xlim([0 500])
