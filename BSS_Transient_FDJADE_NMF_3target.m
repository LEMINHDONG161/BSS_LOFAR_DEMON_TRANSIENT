...
cd(Data_path)
disp('Load...')
load('transient'); Target_1 = Target_sig;
load('Target_N1'); Target_2 = Target_sig;
load('Target_N2'); Target_3 = Target_sig;
...
Tar = [Target_1; Target_2; Target_3];
R = [R1; R2; R3;R4];
...
Ds_T_deci = [Ds_T_deci_1; Ds_T_deci_2; Ds_T_deci_3]; % lofar (Resampling)
...
Ds_T_STFT = cell(1,2,3); 

Ds_T_STFT{1,1} = Ds_T_STFT1; Ds_T_STFT{1,2} = Ds_T_STFT2;Ds_T_STFT{1,3} = Ds_T_STFT3; % LOFAR (STFT)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ds = R;
...
Ds_deci_4 = decimate(Ds(4,:),sc); % Down sampling 3
Ds_deci = [Ds_deci_1; Ds_deci_2; Ds_deci_3;Ds_deci_4]; %  (Resampling)
...
Ds_STFT = cell(1,2,3,4); 

Ds_STFT{1,1} = Ds_STFT1; Ds_STFT{1,2} = Ds_STFT2; Ds_STFT{1,3} = Ds_STFT3;Ds_STFT{1,4} = Ds_STFT4;% LOFAR (STFT)
...
[FD_sep3, T_ifft] = istft(FD_sep3_F, win, overlap, nfft, dfs);
...
for ii = 1:size(Ds_T_STFT,3)
    subplot(3,1,ii)
      plot(F_LOFAR_T,(abs(Ds_T_STFT{1,ii}./max(abs(Ds_T_STFT{1,ii})))));
    %caxis([-20 0])
    xlim([0 500])
    %ylim([0 500])
...
figure,
set(gcf,'numbertitle','off','name', ' Raw Time information Target signal');
  
 plot(T_LOFAR_T,Ds_T_STFT{1,1},'r-');
   
  xlim([0 180])
  %ylim([0 1000])
  xlabel('Time (Sec)','fontsize',12); ylabel(' ','fontsize',12);
  set(gca,'fontsize',12)
  set(gcf,'color','w')

%figure,
%set(gcf,'numbertitle','off','name', 'Frequency information Target signal');

    %plot(F_LOFAR_T,Ds_T_STFT{1,1},'r-');
    %caxis([-60 0])
    %xlim([0 500])
    %ylim([-1000 1000])
    %xlabel('Frequency','fontsize',12); ylabel('','fontsize',12);
    %set(gca,'fontsize',12)
    %set(gcf,'color','w')
...
figure,
set(gcf,'numbertitle','off','name', 'Recieved signal3');
surf(T_LOFAR,F_LOFAR,10*log10(abs(Ds_STFT{1,4}./max(abs(Ds_STFT{1,4}))))...
        ,'edgecolor','none'); axis tight;
    shading interp;
    view(0,90);
    colorbar;
    ...
    set(gcf,'color','w')
figure,
set(gcf,'numbertitle','off','name', ' frequency  received signal');
for ii = 1:size(Ds_STFT,4)
    subplot(4,1,ii)
      plot(T_LOFAR,(abs(Ds_STFT{1,ii}./max(abs(Ds_STFT{1,ii})))));
    %caxis([-20 0])
    xlim([0 180])
    %ylim([0 500])
...
figure,
set(gcf,'numbertitle','off','name', ' frequency  FDJADE');
for ii = 1:size(FD_sep_F,3)
    subplot(3,1,ii)
      plot(F_LOFAR_T,(abs(FD_sep_F{1,ii}./max(abs(FD_sep_F{1,ii})))));
    %caxis([-20 0])
    xlim([0 500])
    %ylim([-30 0])
    xlabel('Frequency','fontsize',12); ylabel('Amplitude','fontsize',12);
...
set(gca,'fontsize',12)
set(gcf,'color','w')
%% Display 20s & 40s NNMF information
figure,
plot(F_LOFAR,-db(XmagHat1(:,10)),'b-');
...
set(gcf,'color','w')
grid on
%% Display 20s  NNMF-NNMF & FDJADE information
figure,
plot(F_LOFAR,db(XmagHat1(:,10)),'r-');
%plot(F_LOFAR_T,db(XmagHat./max(max(XmagHat)))); % display for target input nnmf
xlabel('Frequency hz ','fontsize',12);
ylabel('Amplitude ','fontsize',12);
xlim([0 500])
...
%% Display 40s NNMF _ FDJADE & NNMF information
figure,
plot(F_LOFAR,db(XmagHat1(:,20)), 'r-');
%plot(F_LOFAR_T,db(XmagHat./max(max(XmagHat)))); % display for target input nnmf
xlabel('Frequency hz ','fontsize',12);
...
xlabel('Frequency W ','fontsize',12);
%ylabel('Amplitude ','fontsize',12);
xlim([0 500])
%ylim([0 0.3])
set(gca,'fontsize',12)
