...
disp('FD-JADE...')
FD_sep1_F = []; FD_sep_F = cell(1);
for i = 1:size(Ds_STFT{1,1},2)
    ST = [Ds_STFT{1,1}(:,i)'];
    [Ae, FD_jade] = jade(ST,1);
    FD_sep1_F = [FD_sep1_F FD_jade(1,:)'];
   
end
clear i

FD_sep_F{1,1} = FD_sep1_F;  % STFT (Seperated)

% IFFT
[FD_sep1, T_ifft1] = istft(FD_sep1_F, win, overlap, nfft, dfs);

FD_sep = [FD_sep1]; % Seperated signals
...
figure,
plot(F_LOFAR,db(FD_sep_F{1,1}(:,10)),'r-');
...
grid on
figure,
plot(F_LOFAR,db(FD_sep_F{1,1}(:,80)), 'g-');
%plot(F_LOFAR_T,db(XmagHat./max(max(XmagHat)))); % display for target input nnmf
xlabel('Frequency hz ','fontsize',12);
ylabel('Amplitude ','fontsize',12);
xlim([0 500])
