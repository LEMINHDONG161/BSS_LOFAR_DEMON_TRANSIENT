...
cd(Data_path)
disp('Load...')
load('Target_nnf1'); Target_1 = Target_sig;
load('Target_nnf1'); Target_2 = Target_sig;
...
figure,
surf(T_LOFAR,F_LOFAR,db(XmagHat./max(max(XmagHat))),'edgecolor','none'); axis tight;
shading interp;
view(0,90);
colorbar;
caxis([-20 0])
...
grid on
figure,
plot(T_LOFAR,(XmagH1./max(max(XmagH1))));
...
grid on
figure,
plot(T_LOFAR,(XmagH2./max(max(XmagH2))));
...
grid on
figure,
plot(T_LOFAR,(XmagHat./max(max(XmagHat))));
...
grid on
figure,
plot(F_LOFAR,(W1./max(max(W1))));
xlabel('Frequency W1 ','fontsize',12);
ylabel('Amplitude','fontsize',12);
%xlim([0 500])
%ylim([-10 0])
set(gca,'fontsize',12)
...
grid on
figure,
plot(F_LOFAR,(XmagH1./max(max(XmagH1))));
...
grid on
figure,
plot(F_LOFAR,(XmagH2./max(max(XmagH2))));
...
grid on
figure,
plot(F_LOFAR,(XmagHat./max(max(XmagHat))));
